...
```
Si hablamos de lo que entrega la función residualPlots, podemos ver los parametros de Waist.Girth y Chest.Girth se ven que siguen una estabilidad con una curvatura en el centro. En el caso de Navel.Girth, se ve una mayor disperción en comparación a los 2 anteriores. En base a lo anterior,se puede decir que no hay patrones claros que indiquen heterocedasticidad.
Ahora bien, si analizamos los valores de p obtenidos por cada paremetro, son mayores al valor que uno define comunmente (0.05) y dando a entender que es un modelo adecuado, que no presenta problemas de multicolinealidad y que los parametros son significativos.
...
Dado al los valores que nos entregan, el modelo presenta un rendimiento bajo, con una exactitud baja y un error alto. La sensibilidad y especificidad son bajas, lo que nos indica que el modelo no es capaz de predecir correctamente el estado nutricional de los individuos.